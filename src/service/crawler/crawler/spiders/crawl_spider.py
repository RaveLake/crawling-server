...
class DefaultSpider(scrapy.Spider):
    ...
    def _data_verification(self, response, item: Dict[str, List]):
        ...
        if link_len == 0:
            ...
            if self.try_time < 2:
                ...
                raise Exception(
                    f'{when}, Crawled item is empty! Xpaths and base url is wrong or connection has failed.')
    ...
    def set_args(self, board_data: BoardData):
        self.model = board_data.model_name
        self.key_name_bid = board_data.key_name_bid
        self.url_xpath = board_data.uri_link
        titles_xpath = board_data.uri_title
        is_fixed = board_data.uri_is_fixed
        dates_xpath = board_data.uri_date
        authors_xpath = board_data.uri_author
        references_xpath = board_data.uri_reference
        self.in_date_xpath = board_data.uri_inside_date
        self.drop = board_data.notice_drop_num
    ...
    def split_id_and_link(self, links: List[str]) -> Tuple[List[str], List[str]]:
        ...
        for link in links:
            urls.append(link + '&')
        for url in urls:
            if self.key_name_bid == 'restful':
                idx = url.rfind('/') + 1
                if url[idx] == '?':
                    idx = url[:idx - 1].rfind('/') + 1
            else:
                idx = url.rfind(self.key_name_bid + '=') + len(self.key_name_bid) + 1
